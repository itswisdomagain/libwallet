...
// resCResponse will return a payload the consumer, and log it if possible.
func resCResponse(payload string) *C.char {
	...
	logMtx.RLock()
	if log != nil {
		log.Debugf("returning payload to consumer: %v", payload)
	}
	logMtx.RUnlock()
	return cString(string(b))
}
