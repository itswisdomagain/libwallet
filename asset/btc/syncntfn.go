...
func (w *Wallet) monitorSyncActivity(ctx context.Context, monitorProgress bool) {
	select {
	...
	if monitorProgress {
		w.handleAllSyncNtfnUpdates(ctx)
	} else {
...
func (w *Wallet) handleAllSyncNtfnUpdates(ctx context.Context) {
	for {
		select {
		case n, ok := <-w.ChainClient().Notifications():
			...
			case chain.BlockConnected:
				...
				w.syncer.HandleBlockConnected(n.Height, nil)
			...
			case chain.FilteredBlockConnected:
				...
				w.syncer.HandleBlockConnected(n.Block.Height, n.RelevantTxs)
