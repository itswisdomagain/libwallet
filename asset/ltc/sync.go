...
func (w *Wallet) StartSync(ctx context.Context, reportProgress bool, connectPeers []string, savedPeersFilePath string) error {
	...
	chainClient := chain.NewNeutrinoClient(w.chainParams, chainService, &assetlog.BTCLogger{Logger: w.log})
	...
	peerManager := asset.NewSPVPeerManager(&ltcChainService{chainService}, connectPeers, savedPeersFilePath, w.log, w.chainParams.DefaultPort)
